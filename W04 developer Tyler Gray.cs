class Journal
{
    ...
    private void AddEntry(JournalEntry entry) => entries.Add(entry);

    private void DisplayEntries() => entries.ForEach(Console.WriteLine);

    private void SaveToFile(string fileName) => File.WriteAllLines(fileName, entries.ConvertAll(entry => $"{entry.Date},{entry.Prompt},{entry.Response}"));
